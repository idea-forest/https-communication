...
public:
	...
	C_REQ(ROLE _role, CMD_TP _tp, CMD_TP _prev, char *_content = 0, char* _bin = 0,int _bin_len = 0, char* _uname = 0) {
	...
	C_REQ(ROLE _role, char* _bin, int _bin_len , char* _uname , int _file_pos) {
		role = _role; tp = UPLOAD; prev_tp = NONE;
		bin = _bin;bin_len = _bin_len;
		uname = _uname;file_index = _file_pos;
	}

	C_REQ(ROLE _role, char* _uname, int _file_pos) {
